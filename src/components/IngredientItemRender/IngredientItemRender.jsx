...
function IngredientItemRender({data}) {
  ...
  const selectedBurgerIngredient = useSelector(state => state.ingredientItemRenderReducer.selectedIngredient);
  const burgerIngredients = useSelector(state => state.burgerConstructorReducer);
  ...
  const ingredientsCounter = React.useMemo(() => {
    return data.type === 'bun' && data._id === burgerIngredients.bun._id
      ? 2
      : burgerIngredients.ingredients.filter((item) => item._id === data._id).length;
  }, [burgerIngredients.ingredients, burgerIngredients.bun, data]);
    ...
    <>
      <li className={ingredientItemRenderStyles.listItem} key={data._id} onClick={openModal} ref={dragRef}>
        ...
        {ingredientsCounter > 0 &&
          <Counter count={ingredientsCounter} size='default' extraClass={ingredientItemRenderStyles.counter}/>}
      ...
      </li>
      {
        ...
        <Modal closeModal={closeModal}>
          <IngredientDetails data={selectedBurgerIngredient}/>
        </Modal>
      }
    </>
  );
